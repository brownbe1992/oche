...
**Around the Clock leaderboards** (switching the toggle to Around the Clock):
- *Practice tab* — Fastest Completion (each player's own fastest completed solo run, by darts) and Most Completions (total completed solo runs). Races don't appear here: clearing twenty numbers while somebody else is throwing is a different feat from clearing them fastest alone.
- *H2H tab* — Most Race Wins, ranking head-to-head races won.
...
| **Checkout Trainer sub-mode** (Checkout Trainer only) | Freeform (untimed) · ⏱️ Checkout Blitz (60 seconds) · 🗺️ Route Recall |
| **Route Recall dart limit** (Route Recall only) | 1 dart · up to 2 darts · up to 3 darts |
...
**🧮 Checkout Trainer** is a pure mental-recall drill, not a throwing game — no dartboard is involved at all. The app gives you a target score and you tap out your proposed checkout (up to 3 darts) on the same Pad you already use everywhere else (the Dartboard input is hidden for this mode — with no real throw to record, a mistap on the board's miss ring would silently turn a correct route into an illegal one); on submit it's graded instantly: ✅ **Optimal** (the objectively fewest possible darts), ⚠️ **Legal, not optimal** (a valid finish, just not the shortest route), or ❌ **Not a legal finish**. Anything short of optimal reveals the best route so you actually learn something from every attempt, not just get scored. This is deliberately different from Daily Challenge's **Checkout Sprint** format, which measures a real physical throw at a real target — Checkout Trainer never involves a real dart, it tests checkout *knowledge*, not throwing performance. Nothing it records counts as a dart thrown anywhere else in the app — not toward your darts-thrown totals, your averages, your dartboard heatmap, or any Personal Best. That isn't a filter applied stat by stat: the mode keeps its history in a table of its own and writes no darts at all.
...
Checkout Trainer has its own 40-badge set (33 laddered milestones across 6 ladders — Lifetime Attempts, Lifetime Optimal Answers, Session Endurance, Best Optimal Streak, Checkout Blitz's own Best Blitz Score, and Route Recall's Routes Named — plus 7 one-off badges: 🐟 The 170 Club, 🎯 One-Darter, 🌟 Perfectionist, 💎 Perfect Minute, 📸 Photo Finish, 💣 Bogey Buster for a first correct "no possible checkout" call with trick questions on, and 🗺️ Cartographer for finding every route of a target that has ten or more). Like Just Chuckin' It's milestones, every laddered badge here is a permanent, once-earned achievement. See [Achievements & Badges](#achievements--badges) below.

**🧭 Around the Clock** is a guided drill you can play alone or as a race: hit every number 1 through 20 as a single, in any order. A live progress grid on the scoring screen and Live Scoreboard shows exactly which numbers are still outstanding, updating after every dart. The clock is complete the instant all 20 are hit — the game ends right there with a **Clock Complete** results screen (darts thrown, trebles, doubles, misses, and time taken) on both the scoring screen and the Live Scoreboard, plus the same **🔁 Play Again**/**🏠 Return Home** options every other completed game gets. There's no numeric score, no opponent, and no Enter-turn step, same as Doubles Practice/Just Chuckin' It — every dart commits the instant it's thrown, and Undo Last Dart is supported. The first time you ever complete a clock, you earn the **Guided Clock** badge.

Add a second player (or a third, or a fourth) and it becomes a **race** instead: same rules, but three darts each, turn about, and the first to clear all twenty takes the leg — with real best-of-N legs and sets, like any other head-to-head game. Everyone gets their own progress grid on the scoring screen and their own lane on the Live Scoreboard, so you can see at a glance who's ahead and on which numbers. A race counts toward head-to-head win rates and Household Ratings, and has its own Most Race Wins leaderboard; it never counts toward the two solo drill boards. See [Player Profile](#player-profile) for its own stat bubbles (Completions, Darts/Completion, Darts Thrown) and Personal Bests (fastest completion), and the Home page for its own leaderboards.

**🗺️ Around the World** is the same idea at full scale: hit all 63 dart outcomes (every number 1–20 as a single, double, and treble, plus outer bull, double bull, and a miss) **in one session**. Like Around the Clock, one world is one game — the live grid is your checklist for this session, starting empty every time, and completing it ends the game with a **WORLD COMPLETE** screen and earns the **Guided World** badge. Your all-time progress is still tracked and shown alongside it (a dart you've never hit in your life is called out as such), and the separate passive **Around the World** badge for the lifetime set keeps firing from any mode. You can also just press **End game** whenever you've had enough. See [Player Profile](#player-profile) for its own stat bubbles (Sessions Played, Darts Thrown) and Personal Bests (sessions played, lifetime progress), and the Home page for its own leaderboard.
...
**💀 Dead Man Walking** is a solo drill built entirely from your own weak spots: **15 rounds**, each one dropping you mid-checkout on one of *your own* historically toughest X01 finishes (drawn from real double-out history in the 32–170 range, weighted by how often you've busted or failed to close that exact number — see `REFERENCE.md` for the exact formula), with a fixed **dart budget** set by the difficulty you choose at setup — **Easy** (+6 darts beyond a perfect checkout), **Medium** (+4, the default), **Hard** (+2) or **Extreme** (the perfect route exactly, no margin). A 170 gives you 9 darts on Easy and 3 on Extreme. Every round is a real X01-shaped visit, double out always, bust rules included — check it out before the budget runs dry and you've **Walked Out**; bust, or run out of darts first, and you're **Executed**. Your 15 targets are computed once, server-side, from a snapshot of your own history the moment the run starts, then locked for the whole session — there's no live recalculation and no way to influence which numbers you get. The budget comes from your chosen difficulty and is the same for everyone at that setting. A player without enough double-out history yet instead draws from the same curated checkout pool Daily Challenge uses, so the drill works from the very first run. The scoring screen shows the live round (N of 15), your deficit, a dart-count countdown to the budget, and your running Walked Out tally; Save Game mid-run and Undo Last Turn are both supported. How many of the 15 you Walk Out of lands on a result from **Executed** up through **Pardoned**. See [Player Profile](#player-profile) for its own stat bubbles (Runs Completed, Avg Walked Out/Run, Bust Rate, Out-of-Darts Rate, Avg Margin, Longest Walked-Out Streak), a Personal Best (Most Walked Out in a Run), the Home page for its own arcade-style high-score leaderboard (single best-ever run, no minimum floor), and 14 achievements (three milestone ladders — lifetime runs completed, lifetime rounds Walked Out, and a longest Walked-Out streak that can span across runs — plus 🕊️ Full Reprieve, ⚰️ Pardoned, and 💀 Last Request).
...
**🧠 Maths Trainer** is a Minigame for anyone who hasn't learned the doubles and trebles of the higher numbers — no dartboard, no darts, playable on a train. Two skills in order: know what treble 19 *is* off the top of your head, then total a whole visit at a glance. You get a question and four options, one tap, and it is graded on **speed as well as correctness** — because a right answer you had to work out means you computed it rather than knew it, which is exactly the habit this removes. Answer inside the threshold and it counts as *known*; answer correctly but slowly and it says so. Your headline stat is how many segments you know cold, and every session ends on a **crib sheet** listing each segment with its value, how fast you answer it, and which ones you still stall on. Segment recall runs on the 22 doubles/trebles of 10–20 (Hard adds the bull and the awkward low doubles); visit counting gives you two or three darts, either named in words or — the real skill — **drawn on a board for you to read**. There's a 60-second **Sprint** with its own leaderboard, five achievement ladders and three one-off badges. Nothing it records touches any other statistic: it writes to its own table, so a typed-in answer can never reach an average, a dart count or a leaderboard belonging to a game you actually threw.
...
- **H2H (X01):** the same layout, one hero card per player with their three-dart average (winner first, marked 🎯), the leg's or match's best visits as a chip shelf, tallies for ton+ visits / 180s / best checkout / busts, and each player's own stat column with their legs/sets standing.
- **Every other game mode** gets the same panel, built around whatever that mode's headline number actually is: **Cricket** leads with **MPR** and shows the seven numbers with who closed each; **Bob's 27** shows your final score and all twenty doubles, each marked with what it gained or cost; **The Gauntlet** shows total Scars and all twenty stations, clean or scarred; **Dead Man Walking** shows how many of the fifteen rounds you walked out of, with each round's target and the darts it took; **Baseball**, **Shanghai**, **Halve-It** and **The Pressure Chamber** each show their running total and the winner's own round-by-round card; **Killer** shows lives left (or OUT) and the whole table; **Around the World** shows darts-to-63 and each number's three outcomes; **Around the Clock** shows darts and time for a solo run, with no card — every one of its twenty numbers is hit by definition once the clock ends — and for a race shows each racer's darts alongside the twenty numbers and who got to each one. Every cell that carries a state says it in words as well as colour ("clean", "executed", "all closed"), so nothing depends on colour alone.
- Solo drills end on **"Session complete"**, not "Ben wins the game" — the same reasoning as the practice banner above.
- Just Chuckin' It, Doubles Practice and Checkout Ladder have no completion screen at all: they have no round or leg boundary that ends a game, so they run until you tap End Game. Checkout Trainer keeps its own checkout-card results screen.
...
Boards get rotated every so often to spread the wear around, and the number ring
moves with them. Rotate by an odd number of sectors and your 20 ends up on the
other colour scheme. This setting keeps the on-screen board matching the one on
your wall.

Look at your board's 20 and pick the bed colour it sits on:

| | Single bed | Double & treble |
|---|---|---|
| **Black bed** | black | red |
| **White bed** | white | green |

Every other sector alternates to the other scheme automatically, and there's a
live preview of the whole board. The change applies immediately, including to a
game already in progress.

If **Colorblind mode** (under Accessibility) is on, the doubles and trebles still
render in its orange and blue; the preview shows exactly what will be drawn.
...
> rings**, which is what an unrotated dartboard looks like. Previously the board
> paired the *tan* bed with the red ring — a combination no real board has —
> because the beds and the rings alternated independently of each other. The green
> is also a shade darker so the "Bull" label stays legible on a rotated board.
...
GET  /api/settings/board-colors             { sector20, even, odd } — public, read at app boot;
