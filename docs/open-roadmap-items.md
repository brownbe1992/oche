...
| 11b | New Game page revamp: replace the single all-controls-visible setup screen with a 3-step wizard (Who's playing? → Choose a game → More options) — consolidates the Mode row/Practice-type sub-toggle/X01-Cricket toggle into one game-type dropdown, adds how-to-play copy for every mode, no data-model changes | `docs/new-game-flow-roadmap.md` | Medium-High |
