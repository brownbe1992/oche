...
| 3 | Dart Builder: optional photo upload per component (a real photo of the actual barrel, instead of the generic icon set — see the Done ledger) | `docs/dart-builder-roadmap.md` | Low |
| 4 | Mobile: native chrome — change-server access, haptics, biometric unlock (step 4) | `docs/mobile-app-roadmap.md` | Low-Medium |
| 5 | Mobile: distribution decision — App Store/Play Store listing vs. simpler sideload distribution (step 6) | `docs/mobile-app-roadmap.md` | Low-Medium |
| 6 | Localize voice announcements beyond hardcoded English phrases | `docs/voice-announcements-i18n-roadmap.md` | Low-Medium |
| 7 | Tournament: two new badges — 🏆 Champion (win a bracket) and a seed-based Giant Slayer equivalent (beat a meaningfully higher seed) | `docs/tournament-mode-roadmap.md` | Low-Medium |
| 8 | Tournament: stats on the Player Profile (wins, runner-up count, best finish reached) — the doc's own step 4 stretch goal, expanded into a full design | `docs/tournament-mode-roadmap.md` | Low-Medium |
| 9 | Dartboard zone/miss/bounce-out tracking — a nullable `darts.zone` column so heatmaps can tell an inner single from an outer single instead of one merged bucket; generalizes Just Chuckin' It's dartboard-shaped heatmap (today Chuckin-exclusive) to X01, Cricket, and Doubles Practice; replaces Dartboard mode's flat Miss button with a two-band (near/far) positional miss ring; adds a flat "Bounce Out" button (both input modes, v1 tracks count only — positional capture is a v2 explicitly gated on `docs/camera-scoring-roadmap.md`, since manually reconstructing where a dart struck after it fell isn't reliable data) for darts that struck the board but didn't stay | `docs/dartboard-zone-tracking-roadmap.md` | Medium |
| 10 | Mobile: Capacitor scaffold (iOS + Android) with the native Server Setup screen (step 2) | `docs/mobile-app-roadmap.md` | Medium |
| 11 | Mobile: ATS/cleartext config + self-signed cert trust-prompt (step 3) | `docs/mobile-app-roadmap.md` | Medium |
| 12 | League mode (new tables, no new infra; complements tournament mode) | `docs/league-mode-roadmap.md` | Medium |
...
| 14 | Guided Around the Clock / Around the World practice drill mode — turns the existing passive completion tracking into a fourth Practice Drill Mode with live progress feedback, reusing Around the World's existing heatmap/progress-view UI | `docs/game-modes-roadmap.md` | Medium |
| 15 | Per-player data export (CSV + JSON, PIN-gated) — re-opened with fresh product direction after being explicitly descoped when the admin full-database export shipped; design was already fully written, just shelved | `docs/data-export-roadmap.md` | Medium |
| 16 | Game Modes: Baseball — the second proof that the plugin shape generalizes beyond Cricket (step 5) | `docs/game-modes-roadmap.md` | High |
| 17 | Tournament mode: double-elimination bracket support (losers bracket + grand final/reset logic, the genuinely fiddly combinatorial piece — single-elimination already shipped, see the Done ledger) | `docs/tournament-mode-roadmap.md` | High |
...
- **The small, order-independent items** (rows 3, 6-8) can be interleaved anywhere, including ahead of the bigger lifts — good for sustaining momentum with essentially zero risk of creating rework later.
...
| Dart Builder v1: `dart_components`/`loadouts` schema, component/loadout CRUD, `game_players.loadout_id` game-creation integration (barrel weight snapshot, retires `players.dart_weight` as a write path), PIN-gated default-loadout + loadout-customization actions, per-loadout stats (games/wins/darts/avg/180s/checkouts), Dart Builder screen (loadout list + editor), Player Profile "Default Loadout" selector, New Game "Change Loadout" picker with auto-default selection, Add Player modal's Dart Weight dropdown removed — photo upload remains a separate open item above | `docs/dart-builder-roadmap.md` |
| Dart Builder accessibility icon set: barrel shape/grip and flight shape are now an accessible icon-button group (`role="group"`, per-button `aria-pressed`, decorative `aria-hidden` SVGs) instead of text-only dropdowns, closing the "torpedo"/"knurled"/"kite" self-explanatory-name gap `docs/accessibility-roadmap.md` flagged; the underlying hidden-input value contract kept the existing submit path unchanged | `docs/dart-builder-roadmap.md` |
| Dart Builder "quick-add full set" — a "⚡ Quick Add Full Set" screen enters a loadout name plus every barrel/shaft/flight field and saves all of it in one action (client-side orchestration of the same `createComponent()`/`createLoadout()` calls the normal 3-modal flow makes, no new backend endpoint), for logging an off-the-shelf dart set without three separate round trips | `docs/dart-builder-roadmap.md` |
| Dart Builder: loadout comparison view — a "⚖️ Compare Loadouts" button (2+ loadouts) opens a side-by-side stats table (components/games/wins/win%/darts/average/180s/checkouts), reusing the existing `getLoadoutStats()` once per loadout with no new backend query; toggling a loadout in/out of the table re-renders from a per-screen-visit cache, never re-fetches | `docs/dart-builder-roadmap.md` |
