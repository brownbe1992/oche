...
| 10a | Data export: re-import (taking an exported per-player JSON back into a different Oche instance) — the shipped export's `uuid`+opponent-stub design was built to make this lossless once attempted, but import itself isn't built; recommended shape (look up opponents by uuid first, auto-create a stub if missing, merge-on-match deferred as its own harder problem) is written up but not implemented | `docs/data-export-roadmap.md` | Medium |
| 10b | Data export: CSV export (a simpler, non-portable "your own stats as a spreadsheet" format, one row per turn or per game) — deliberately separate from the shipped per-player JSON export, doesn't need to solve H2H preservation the way JSON does | `docs/data-export-roadmap.md` | Low-Medium |
...
| Per-player data export (admin-only, JSON) — new "Export a player…" admin page (Settings → Data Export), `getPlayerExport()`/`GET /api/players/export`; bundles games/turns/darts for every game the player is in, including opponents' turns within those same games (H2H isn't stored anywhere, only derivable) plus minimal `{uuid,name}` opponent stubs; new `players.uuid` (v4 UUID) portable identity backing it; committed tests — re-import and CSV export are separate, not-yet-started items above | `docs/data-export-roadmap.md` |
