...
| 12 | Game Modes: Baseball — achievements/badges, matchwin moment card + Share button, per-leg practice stat panel, and Home page leaderboards — the core playable game (2026-07) and Player Profile stats parity (stat bubbles/Personal Bests/metric history, 2026-07) both shipped, see the Done ledger below | `docs/game-modes-roadmap.md` | Medium |
...
| Game Modes: Baseball — Player Profile stats parity (RPI/Perfect Innings/Win Rate/Games Played/Darts Thrown/Best Inning stat bubbles, 5-field Personal Bests, 6 metric-history cases, `getBaseballWonLegs()`'s query-time leg-winner derivation in place of a stored `leg_won` signal). Achievements and Home page leaderboards remain open, tracked as item #12 | `docs/game-modes-roadmap.md` |
