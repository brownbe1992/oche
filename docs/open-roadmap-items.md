...
| 12b | Game Modes: Killer — elimination-style, per-player random number assignment, per-dart evaluation for becoming a "killer" and attacking opponents; needs its own config shape (`config.numbers` as a per-player map), scoring screen, and stat vocabulary; several rules variants (lives-per-hit, friendly fire) need a decision before the turn engine can be written | `docs/game-modes-roadmap.md` | High |
