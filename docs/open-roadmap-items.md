...
| 13 | Checkout Trainer — a no-dartboard mental drill: given a target score, enter the fewest-darts checkout from memory, graded instantly via the existing `checkoutHint()` calculator; two modes (untimed Freeform and a 60-second Checkout Blitz sprint with its own leaderboard), laddered achievements for both; no live scoreboard needed | `docs/checkout-trainer-roadmap.md` | Medium |
