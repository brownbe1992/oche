...
| `docs/game-modes-roadmap.md` | Partial | Very high | Very high | Steps 1-3 done: X01-to-plugin refactor, Cricket fully playable (turn engine, New Game classic/custom config, dedicated scoring screen, chalkboard scorecard live scoreboard), and Cricket stats parity (MPR + 5 more stat bubbles, 9-Marks leaderboard, Personal Bests via new `turns.leg_won` column, 2 achievements). Step 4 partial (Player Profile's X01/Cricket toggle done; Home page leaderboards still X01-only). Step 5 (Baseball) not started |
