...
| `docs/accessibility-roadmap.md` | Partial | Low-Medium | High | 3 of 5 items done (colorblind mode, the WCAG contrast audit, `aria-live` announcements on the controller). The contrast audit (2026-07) found and fixed 4 real AA failures — `--green`/`--bust` brightened, a new `--red-text` for the Pad's "Bull" label, and a colorblind-mode-specific fix to the dartboard's own "Bull" center label. Accessible-input framing and a type-size pass remain outstanding — standing checklist per CLAUDE.md, not a one-off |
