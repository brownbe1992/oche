...
| `docs/game-modes-roadmap.md` | Partial | Very high | Very high | Steps 1-2 done: X01-to-plugin refactor + schema groundwork, and Cricket is now fully playable (turn engine, New Game classic/custom config, dedicated scoring screen, orientation-aware live scoreboard card). Steps 3-5 (Cricket stats parity/achievements, Home/Profile game-type nav, Baseball) not started — full scoring-engine + stats-pipeline generalization for those, not just "add Cricket" |
